...
def test_bacon_number_03():
    ...
    assert (
        len(lab.actors_with_bacon_number(
            lab.transform_data(make_bacon_tree(N, k)), N))
        == k
    )
...
def test_bacon_number_04():
    ...
    assert len(lab.actors_with_bacon_number(
        lab.transform_data(make_bacon_tree(N, k)), 10**20)) == 0
    assert len(lab.actors_with_bacon_number(
        lab.transform_data(make_bacon_tree(N, k)), 10**20)) == 0
...
def check_valid_path(f, p, s, e, l):
    ...
    assert lp == l + \
        1, f"expected a path of length {l+1} between {s} and {e}, got {lp}"
