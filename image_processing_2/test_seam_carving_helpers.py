...
def test_greyscale():
    for fname in ('centered_pixel', 'smallfrog', 'bluegill', 'twocats', 'tree'):
        ...
        expfile = os.path.join(
            TEST_DIRECTORY, 'test_results', f'{fname}_grey.png')
        assert object_hash(
            im) == oim, 'Be careful not to modify the original image!'
...
def test_pattern_cumulative_energy():
    ...
    assert object_hash(
        energy) == oim, 'Be careful not to modify the original energy!'
    compare_greyscale_images(result, expected)
...
def test_pattern_seam_indices():
    ...
    expected = [2, 11, 21, 31]
    assert object_hash(
        cem) == oim, 'Be careful not to modify the original cumulative energy map!'
    assert (set(result) == set(expected))


def test_min_seam_indices():
    for fname in ('centered_pixel', 'smallfrog', 'bluegill', 'twocats', 'tree'):
        infile = os.path.join(TEST_DIRECTORY, 'test_results',
                              f'{fname}_cumulative_energy.pickle')
        with open(infile, 'rb') as f:
            cem = pickle.load(f)
        result = lab.minimum_energy_seam(cem)
...
def test_seam_removal():
    for fname in ('pattern', 'bluegill', 'twocats', 'tree'):
        infile = os.path.join(TEST_DIRECTORY, 'test_results',
                              f'{fname}_minimum_energy_seam.pickle')
        with open(infile, 'rb') as f:
            seam = pickle.load(f)
        ...
        expfile = os.path.join(
            TEST_DIRECTORY, 'test_results', f'{fname}_1seam.png')
        compare_color_images(result, lab.load_color_image(expfile))
