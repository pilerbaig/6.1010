...
def test_blurred_black_image(kernsize):
    ...
    input_img = {"height": 6, "width": 5, "pixels": [0] * 30}
    input_hash = object_hash(input_img)
    result = lab.blurred(input_img, kernsize)
    expected = {"height": 6, "width": 5, "pixels": [0] * 30}
    assert object_hash(
        input_img) == input_hash, "Be careful not to modify the original image!"
    compare_images(result, expected)
...
def test_blurred_centered_pixel(kernsize):
    ...
    input_img = lab.load_greyscale_image(os.path.join(
        TEST_DIRECTORY, 'test_images', 'centered_pixel.png'))
    input_hash = object_hash(input_img)
    result = lab.blurred(input_img, kernsize)
    if kernsize == 1:
        expected = {"height": 11, "width": 11, "pixels": [0] * 121}
        expected["pixels"][60] = 255
    elif kernsize == 3:
        expected = {"height": 11, "width": 11, "pixels": [0] * 121}
        for i in [48, 49, 50, 59, 60, 61, 70, 71, 72]:
            expected["pixels"][i] = int(1/9 * 255)
    assert object_hash(
        input_img) == input_hash, "Be careful not to modify the original image!"
    compare_images(result, expected)
...
def test_edges_centered_pixel():
    ...
    result = lab.edges(input_img)
    expected = {'height': 11, 'width': 11, 'pixels': [0] * 121}
    for i in [48, 49, 50, 59, 61, 70, 71, 72]:
